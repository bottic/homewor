...
def move_doc_to_shelf(user_doc_number, user_shelf_number):
    shelf_rdoc = remove_doc_from_shelf(user_doc_number)
    shelf_adoc = append_doc_to_shelf(user_doc_number, user_shelf_number)
    print('Документ номер "{}" был перемещен на полку номер "{}"'.format(user_doc_number, user_shelf_number))
    return shelf_rdoc, shelf_adoc
...
if __name__ == '__main__':
    print(add_new_doc(1,11,111,1111))